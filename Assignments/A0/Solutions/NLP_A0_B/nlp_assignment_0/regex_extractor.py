...
def extract():
    ...
    # reading user_details.csv file
    user_df = pd.read_csv('C:/Users/harry/OneDrive/Documents/Northeastern University/CS6120/Assignments/A0/Solutions/NLP_A0_B/nlp_assignment_0/user_details.csv')

    # printing user details as a data frame
    print(user_df)

    # iterating the user details data frame
    for index, row in user_df.iterrows():

        # re to extract the name attribute
        full_name_match = re.search(r".*(?=:)", row["Description"])

        # re to extract the address attribute
        address_match = re.search(r"[0-9].*(St|Blvd)", row["Description"])

        # re to extract the county attribute
        county_match = re.search(r"[A-z]*(?= County)", row["Description"])

        # re to extract the state attribute
        state_match = re.search(r' [A-Z][A-Z](?=\.|,)', row["Description"])

        # re to extract the phone attribute
        phone_match = re.findall(r"[0-9]*-[0-9]*-[0-9]*", row["Description"])

        # re to extract the name email id
        email_match = re.search(r"[A-z0-9_]*@.*(.com|.org)", row["Description"])

        # extracting fname and lname from name values using split() function and storing in first_name and last_name attribute
        if full_name_match:
            full_name = full_name_match.group().split(" ")
            user_df.at[index, "first_name"] = full_name[0]
            user_df.at[index, "last_name"] = full_name[1]

        # getting address value and storing in address attribute
        address = address_match.group() if address_match else ""
        user_df.at[index, "address"] = address

        # getting county value and storing in county attribute
        county = county_match.group() if county_match else ""
        user_df.at[index, "county"] = county

        # getting state value and storing in state attribute
        state = state_match.group() if state_match else ""
        user_df.at[index, "state"] = state

        # getting phone_number value and storing in phone# attribute
        if phone_match:
            for idx, phone_number in enumerate(phone_match):
                user_df.at[index, f"phone{idx+1}"] = phone_number

        # getting email value and storing in email attribute
        email = email_match.group() if email_match else ""
        user_df.at[index, "email"] = email
    ...
    user_df.to_csv('updated_user_details.csv', index=False)    
